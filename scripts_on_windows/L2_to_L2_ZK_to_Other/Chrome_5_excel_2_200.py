...
excel_path =r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx'
...
def zk_orb_OP():
    ...
    if "失败" in info:
        ...
        Do_Excel(excel_path).write(i, "K", "×")
    else:
        Do_Excel(excel_path).write(i, "M", "1")
        # Do_Excel(excel_path).write(i, "K", "·成功")
        Do_Excel(excel_path).write(i, "W", info)
...
for i in range(29, 201):
    ...
    sign = str(Do_Excel(excel_path).read(i, "N"))
    if "1" in sign:  # ZK 转钱
        fail = Do_Excel(excel_path).read(i, "M")
        print(fail)
        if fail != "·1":
            try:
