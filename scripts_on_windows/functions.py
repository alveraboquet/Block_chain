...
def open_clash_dashboard(browser, wait, url_dashboard):
    ...
    time_sleep(15, "先进行测速处理，防止clash节点出错")
    #mac
    # IPs = wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//*[@id="root"]/div/div[2]/div/div[1]/div/ul/li[1]/div/div[2]/div/ul/li[@class="cursor-pointer"]')))
    #win
    IPs = wait.until(EC.visibility_of_all_elements_located(
        (By.XPATH, '//*[@id="root"]/div/div[2]/div/div[1]/div/ul/li[1]/div/div[2]/div/ul/li[@class="can-click"]')))
...
def ip_switcher(browser, wait, url_google):
    ...
    #win
    IPs = wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//*[@id="root"]/div/div[2]/div/div[1]/div/ul/li[1]/div/div[2]/div/ul/li[@class="can-click"]')))
    #mac
    # IPs = wait.until(EC.visibility_of_all_elements_located((By.XPATH, '//*[@id="root"]/div/div[2]/div/div[1]/div/ul/li[1]/div/div[2]/div/ul/li[@class="cursor-pointer"]')))
    for i in range(3, len(list(IPs)) - 11):
        print("进入ip_switcher的for循环")
        switch_tab_by_handle(browser, 0, 0)  # 先切换到 clash 窗口
        ip_num = random.randint(2, len(list(IPs)) - 26)  # 随机取一个 ip 序列
...
def L2_orb_L2_prepare_transfer_coin(browser, wait, L2_ETH_value, from_source, to_destination):
    ...
    # try_tiems = 0
    # while float(L2_ETH_value) - input_value < 0.009 :
    #     point = random.randint(2, 4)  # 小数点最起码要有2位，因为L1的ETH金额一般是两位小数以上
    #     input_value = round(random.uniform((L2_ETH_value) * (0.5), (L2_ETH_value) * (0.8)), point)
    #     try_tiems = try_tiems + 1
    #     if try_tiems == 25:
    #         print("可能是ETH不足")
    #         return 0
    # print("本次随机转账的ETH金额是：", input_value)
