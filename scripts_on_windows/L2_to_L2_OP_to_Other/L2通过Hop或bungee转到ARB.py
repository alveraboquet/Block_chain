...
def L2_bungee_L2(browser, wait, from_source, to_destination):
    ...
    # ======== 选择源账户，目标账户
    transfer_detail = bungee_prepare_transfer_coin(browser, wait, from_source, to_destination, 0.8, 0.85)
...
def L1_or_L2_hop_L2(browser, wait, from_source, to_destination):
    ...
    hop_prepare_transfer_coin(browser, wait, from_source, to_destination, 0.8, 0.85)
    switch_tab_by_handle(browser, 1, 1)  # 切换到小狐狸
    success_or_fail = fox_confirm_L2_swap(browser, wait)
    print(success_or_fail)
    return success_or_fail  # 失败了，orb


def L2_bungee_ARB():
    info = L2_bungee_L2(browser, wait, "Optimism", "Arbitrum")
    if "失败" in info:
...
def L2_hop_ARB():
    info = L1_or_L2_hop_L2(browser, wait, "Optimism", "Arbitrum")
    if "失败" in info:
        Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i, "W", info)
        Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').plain_write(i, "K", "×")
...
while True:
    for i in range(134, 201):
        ...
        if Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i, "K") != "1成功":
            try:
