...
while True:
    for i in range(2, 51):
        ...
        if str(Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i, "E")) == "1":  # ARB 转钱
            if Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i, "G") != "·成功":
                try:
                    ...
                    print(f"{i}出错了")
                    Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').plain_write(i, "G", "出错了")
                    a = random.randint(10, 20)
                    time_sleep(a, f"++++++++++随机等待时间{a}")
                    try:
                        browser.quit()
