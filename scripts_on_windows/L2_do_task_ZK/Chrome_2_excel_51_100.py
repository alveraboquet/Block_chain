...
for i in range(19, 52):
    if str(Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i+49, "N")) == "1" or str(Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i+49, "N")) == "·1":  # zk 在第7列
        print(f"第{i}，{i+49}个号需要做zk")
        try:
            ##=========== 准备浏览器
            which_chrome = 2
            ...
            if "ZK zigzag" in save_record:
                Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i+49, "V", save_record)  #写到账号所在的行，zk_accounts[i] 不用改，就是用 excel 里的行
                Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i+49, "P", "√")
                time_sleep(60, f"{save_record}，日志写入excel成功，60 秒后反向操作")
            else:
                Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i+49, "V", save_record)  # 写到账号所在的行，zk_accounts[i] 不用改，就是用 excel 里的行
                time_sleep(60, f"{save_record}，日志写入excel成功，60 秒后反向操作")
            #===========反向操作
            ...
            ##=========== 使用指定工作表，保存信息到excel
            print(f"----第{i}出错了")
            Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i+49, "P", "出错了")
            time_sleep(6, "出错了")
            browser.quit()
            continue
