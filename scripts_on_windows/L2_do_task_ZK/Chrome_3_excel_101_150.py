...
for i in range(4, 52):
    if str(Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i+99, "N")) == "1" or str(Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i+99, "N")) == "·1":  # zk 在第7列
        print(f"第{i}，{i+99}个号需要做zk")
        try:
            ##=========== 准备浏览器
            which_chrome = 3
            ...
            if "ZK zigzag" in save_record:
                Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i+99, "V", save_record)  #写到账号所在的行，zk_accounts[i] 不用改，就是用 excel 里的行
                Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i+99, "P", "√哈")
                time_sleep(6, f"{save_record}，日志写入excel成功，60 秒后反向操作")
            else:
                Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i+99, "V", save_record)  # 写到账号所在的行，zk_accounts[i] 不用改，就是用 excel 里的行
                time_sleep(6, f"{save_record}，日志写入excel成功，60 秒后反向操作")
            #===========反向操作
