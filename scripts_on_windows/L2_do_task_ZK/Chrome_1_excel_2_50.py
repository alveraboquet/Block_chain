...
for i in range(2, 51):
    if str(Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i, "N")) == "1" or str(Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').read(i, "N")) == "·1":  # zk 在第7列
        print(f"第{i}个号需要做zk")
        try:
            ##=========== 准备浏览器
            which_chrome = 1
            ...
            ##=========== 使用指定工作表，保存信息到excel
            print(f"----第{i}出错了")
            Do_Excel(r'C:\Users\Terry\PycharmProjects\autopy\L2\eth1000_操作后.xlsx').write(i, "P", f"第{i}出错了")
            time_sleep(6, "出错了")
            browser.quit()
            continue
