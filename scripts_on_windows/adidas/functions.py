...
#保存数据到txt
def save_clipper_data(data, filename):
    time.sleep(4)  # 给点延时，剪切板正在写入
...
def main():
    ...
    while True:
        ...
        TIME_OUT = 15  # 设置显示等待的超时时间，尽量设置的长一点，考虑到网络可能缓慢
        ...
        for i in range(1, 8):
            try:
                # 用返回按钮做定位
                back_button = wait.until(EC.visibility_of_element_located(
                    (By.XPATH, "//button[@class='Header_btn__5X1QK Header_btnStartOver__W5nL_']")))
                ...
                print(f"点击问题{i}失败，退出重启")
                browser.quit()
                break   #这里用break，continue不合适，因为要跳出for循环
            # 点击NEXT
            try:
...
#################################
        ...
        try:
            COPY_CODE = wait.until(
                EC.visibility_of_element_located((By.XPATH, "//div[@class='CodeViewer_buttonContainer__6C6Y7']/button[2]")))
            COPY_CODE = wait.until(
                EC.element_to_be_clickable((By.XPATH, "//div[@class='CodeViewer_buttonContainer__6C6Y7']/button[2]")))
